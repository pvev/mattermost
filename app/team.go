...
func (a *App) shouldCreateOnboardingLinkedBoard(c request.CTX, teamId string) bool {
	...
	if err != nil {
		mlog.Error("Error checking the product: ", mlog.Err(err))
		return false
	}

	if !hasBoard {
		mlog.Error("Board product not found")
		return false
	}
...
func (a *App) createOnboardingLinkedBoard(c request.CTX, teamId string) (*fb_model.Board, *model.AppError) {
	...
	if !ok {
		...
		return nil, model.NewAppError("CreateBoard", "not product key found", nil, "", http.StatusBadRequest).Wrap(err)
	}
