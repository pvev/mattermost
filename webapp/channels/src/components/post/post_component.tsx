...
function PostComponent(props: Props) {
    ...
    const handleRevealBurnOnRead = useCallback(async (postId: string) => {
        ...
        if (result?.error) {
            ...
            let errorMessage = 'Failed to reveal message. Please try again.';
            if (result.error.status_code === 404) {
                errorMessage = 'This message is no longer available.';
            } else if (result.error.status_code === 403) {
                errorMessage = "You don't have permission to view this message.";
            }
    ...
    if (post.type === PostTypes.BURN_ON_READ && post.state !== Posts.POST_DELETED && !props.isConsecutivePost) {
        ...
        const revealedByCount = typeof post.props?.revealed_by_count === 'number' ? post.props.revealed_by_count : 0;
        const totalRecipients = typeof post.props?.total_recipients === 'number' ? post.props.total_recipients : 0;
