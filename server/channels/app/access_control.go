...
func (a *App) GetMaskedExpression(rctx request.CTX, expression string, callerID string) (string, *model.AppError) {
	...
	for i := range visualAST.Conditions {
		a.maskConditionValuesWithToken(rctxWithCaller, &visualAST.Conditions[i], cpaGroupID)
	}
...
func (a *App) maskConditionValuesWithToken(rctx request.CTX, condition *model.Condition, cpaGroupID string) {
...
// Submitted expressions contain only the caller's visible values; this restores the rest.
func (a *App) mergeExpressionWithMaskedValues(rctx request.CTX, submittedExpr, storedExpr, callerID string) (string, *model.AppError) {
	...
	// Match by attribute only — operator can change as value counts change (e.g. hasAnyOf→in).
	// Use stored operator to preserve original semantics.
	...
	// Conditions absent from submitted were deleted by the admin — they are dropped.
...
func (a *App) getHiddenValues(rctx request.CTX, stored *model.Condition, cpaGroupID string) []string {
	...
	field, appErr := a.GetPropertyFieldByName(rctx, cpaGroupID, "", fieldName)
	if appErr != nil {
		return extractStringValues(stored.Value) // fail safe: treat all as hidden
	}
