...
func getJobsByType(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// When team_id is provided, query team-scoped jobs via JSONB data match.
	// Falls back to global sync jobs if no team-scoped jobs exist yet.
	if teamID != "" && model.IsValidId(teamID) {
		teamJobs, err := c.App.Srv().Store().Job().GetByTypeAndData(
			c.AppContext,
			c.Params.JobType,
		...
		if err != nil {
			c.Err = model.NewAppError("getJobsByType", "api.job.get_all.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}
